...
指导教师： & \underline{\makebox[6cm]{}} \\[0.4cm]
...
\begin{enumerate}[itemsep=2pt]
  \item \textbf{地图探索}：玩家在教学楼、图书馆、宿舍、操场等四张可点击地图场景间移动，选择当天的行动地点；
  \item \textbf{事件触发}：到达特定地点后，由数据驱动的 JSON 剧本触发对应的剧情分支；
  \item \textbf{剧情对话}：对话框以打字机效果呈现角色台词与立绘，玩家在关键节点做出选择；
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{\module{MapScene} / \module{LocationItem}}：基于 \texttt{QGraphicsScene} 的可交互地图，玩家可点击教学楼、图书馆、宿舍、操场四个地点触发对应事件。
\end{itemize}
